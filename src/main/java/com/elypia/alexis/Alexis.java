...
public class Alexis {
	...
	public static void main(String[] args) throws GeneralSecurityException {
		...
		AnnotationLoader loader = new AnnotationLoader(Alexis.class.getPackage());
		Context context = new ContextLoader(loader).load().build();
		Commandler commandler = new Commandler.Builder(context)
			.addModules(new AlexisModule(commandLine, configuration))
		...
		JDA jda = new JDABuilder(discord.getToken())
			...
			.addEventListeners(new AlexisListener(), new GreetingsListener(), new XpListener())
			.build();
