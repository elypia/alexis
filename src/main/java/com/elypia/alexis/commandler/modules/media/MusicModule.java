...
//	}
	...
	private boolean joinChannel(JDACEvent event) {
		...
		ElyAudioPlayer guildPlayer = getAudioPlayer(event);
	...
	/**
	 * Obtain the {@link ElyAudioPlayer} instance associated with the guild
	...
	private ElyAudioPlayer getAudioPlayer(JDACEvent event) {
		GenericMessageEvent source = event.getSource();
		long id = source.getGuild().getIdLong();

		if (!guildPlayers.containsKey(id))
			guildPlayers.put(id, new ElyAudioPlayer(source.getJDA(), manager));
