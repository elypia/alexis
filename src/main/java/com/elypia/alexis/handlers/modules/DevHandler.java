...
public class DevHandler extends CommandHandler {
    ...
    public void name(AbstractEvent event, String input) {
        jda.getSelfUser().getManager().setName(input).queue(o -> {
    ...
    public void avatar(AbstractEvent event, URL url) throws IOException {
        try (InputStream stream = url.openStream()) {
            Icon icon = Icon.from(stream);

            jda.getSelfUser().getManager().setAvatar(icon).queue(o -> {
    ...
    public String getGuilds() {
        ...
        jda.getGuilds().forEach(o -> joiner.add(o.getName()));
